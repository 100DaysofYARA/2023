...
rule macho_entitlehash
{
	...
	condition:
		...
		/*
			...
			@cs_magic_entitlement + 8 -> @cs_magic_entitlement + 8 + uint32be(@cs_magic_entitlement+4)
		*/
		for any i in (1 .. #cs_magic_entitlement) : (
			console.log(
				...
				hash.md5(
					...
					@cs_magic_entitlement[i] + 8 + uint32be(@cs_magic_entitlement[i] + 4)
				)
			)
		)
}
